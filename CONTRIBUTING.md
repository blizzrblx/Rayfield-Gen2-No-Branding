...
Thank you for helping improve the library. This guide takes a new contributor from a clean checkout to a pull request that is ready for review.

## Before you start

Use the repository's `dev` branch as the base for normal development. Search the [existing issues](the repository issue tracker) and pull requests before starting work. For a substantial change, open or discuss an issue first so the approach can be agreed before implementation.
...
| Path | Purpose |
| --- | --- |
| `src/init.luau` | The library's module entry point. |
...
| `tests/integration/` | Integration specs for library workflows. |
...
Files such as `build/`, `coverage/`, `sourcemap.json`, `roblox.yml`, `globalTypes.d.luau`, `the library.rbxlx`, and `the library Tests.rbxlx` are generated locally and ignored by Git. Do not commit them.
...
To run the same path with library logs enabled:
...
Open `the library Tests.rbxlx` in Roblox Studio. The `ServerScriptService.RunTests` script requires `ReplicatedStorage.TestEZ`, runs `ReplicatedStorage.Tests`, and errors if any TestEZ test fails.
...
- [Report a bug](the repository issue tracker)
- [Propose a feature](the repository issue tracker)
- [Open the issue-form chooser](the repository issue tracker)
...
Report suspected vulnerabilities privately through [GitHub's private vulnerability reporting form](the GitHub private vulnerability reporting form for this repository). Include reproduction steps, affected versions or commits, impact, and any suggested mitigation. Allow the maintainers time to investigate before public disclosure.
...
Every behavior change must include tests where practical and documentation when it changes public APIs, configuration, examples, or user-visible behavior. Standalone public documentation changes belong in [the documentation repository](the documentation repository); link companion documentation work in the pull request when a code change affects public docs. Documentation-only changes should still verify links and commands against the current repository.

## Licensing and attribution

the library is licensed under the Mozilla Public License 2.0. By contributing, you agree your contributions are licensed under the same terms.
