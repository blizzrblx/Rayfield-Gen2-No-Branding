...
The the library maintainers take security reports seriously. Thank you for reporting suspected vulnerabilities responsibly.
...
Submit suspected vulnerabilities through [GitHub's private vulnerability reporting form](the GitHub private vulnerability reporting form for this repository). Reports submitted there are visible only to authorized repository security managers and administrators.
...
- Whether the detected code was an unmodified official the library release.
- The the library commit, release, or file that triggered the detection.
...
- Unauthorized code execution or injection caused by the library.
...
- Bypasses of intended trust or security boundaries within the library.
...
- A vulnerable dependency with a demonstrated, exploitable impact on the library.
...
- Evidence that official the library code has been compromised, tampered with, or distributed from an unauthorized source.
- A detection indicating that an official release performs undocumented or unsafe operations.

A detection does not automatically establish that the library contains a security vulnerability. The maintainers will investigate reproducible reports involving unmodified official code.
...
- Requests to make the library, an executor, or another tool undetectable.
...
- Account enforcement, moderation, suspensions, or bans without evidence that an unmodified official the library release caused an unintended security issue.
...
- Issues in Roblox, an executor, or another third-party service that are not caused by the library.
...
- Use an unmodified official the library release whenever possible.
